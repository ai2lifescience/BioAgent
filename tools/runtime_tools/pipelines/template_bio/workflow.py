...
def fail(message: str) -> None:
    raise SystemExit(f"template_bio: {message}")
...
def write_vcf(
    ...
    with path.open("w", encoding="utf-8", newline="") as handle:
        ...
        handle.write("##source=template_bio_positional_comparison\n")
...
def write_markdown_report(
    ...
    lines = [
        "# Template Bioinformatics Pipeline Report",
        "",
        f"- Run label: {label}",
...
<title>Template Bioinformatics Report — {escape(metrics['sample_id'])}</title>
<style>
...
<body><main>
<h1>Template Bioinformatics Pipeline Report</h1>
...
def main(argv: list[str]) -> int:
    ...
    label = str(config.get("label") or "template_bio")
    ...
    print(f"Template bioinformatics pipeline completed: {paths['report_path']}")
    return 0
