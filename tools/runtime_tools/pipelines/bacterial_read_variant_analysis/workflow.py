...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if len(args) != 1:
        print(
            "Usage: python3 -m bacterial_read_variant_analysis.config_runner CONFIG.yaml",
            file=sys.stderr,
        )
        return 2
    try:
        ...
        print(f"bacterial_read_variant_analysis pipeline error: {error}", file=sys.stderr)
        return 1
...
def _build_metrics(config: PipelineConfig) -> dict[str, object]:
    ...
    return {
        "pipeline": "bacterial_read_variant_analysis",
        "label": config.label,
        "status": "completed",
