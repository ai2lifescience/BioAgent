...
def file_inspection(path: str, max_preview_lines: int=20, context: WorkflowContext | None=None) -> dict[str, Any]:
    ...
    source, display_path = select_workspace_file(context, path)
    result = context.call('file_inspect', _action_file_inspect, {'path': str(source), 'max_preview_lines': max_preview_lines})['result']
