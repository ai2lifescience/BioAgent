"""Collect and summarize files from the latest completed pipeline run."""
...
def _selected_files(
    ...
    # A completion record lives beside pipeline outputs. The workspace listing
    # supplies available files, so the record cannot pull in another session.
    available = {}
    for item in files:
        path = _project_file(str(item.get("path") or ""))
        if path:
            available[path] = item
    completed = []
    for path in available:
        ...
        try:
            ...
            continue
        if not isinstance(result, dict) or not result.get("completed_at"):
            continue
        if requested_pipeline and result.get("pipeline_name") != requested_pipeline:
            continue
        completed.append(result)
    if not completed:
        return "", "", []

    latest = max(completed, key=lambda item: str(item["completed_at"]))
    declared = {
        str(item["path"]): item
        for item in latest.get("output_records", [])
        if isinstance(item, dict) and item.get("path") and item.get("exists", True)
    }
    paths = declared or {str(path): {} for path in latest.get("files", []) if isinstance(path, str)}
    selected = []
    for value, record in paths.items():
        path = _project_file(value)
        if path in available:
            selected.append({**available[path], **record, "resolved_path": path})
    return str(latest.get("pipeline_name", "")), str(latest.get("run_id", "")), selected
...
def collect_pipeline_results(
    ...
    selected_name, run_id, selected = _selected_files(files, requested_pipeline)
    if not selected:
        return {
            "status": "no_results",
