"""Bounded search across text and selectable-PDF workspace files."""
...
def workspace_search(
    ...
    context = ensure_workflow_context(context, "workspace_search")
...
def _select_files(context: WorkflowContext, requested: str | None) -> list[dict[str, Any]]:
    files = context.files
    if not requested:
        return files
...
def _source_path(context: WorkflowContext, item: dict[str, Any]) -> Path | None:
    try:
