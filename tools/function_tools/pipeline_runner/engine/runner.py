...
from tools.function_tools.pipeline_runner.engine.config import (
    load_pipeline_config,
    load_runner_config,
    normalize_config_overrides,
)
from tools.function_tools.pipeline_runner.engine.inputs import (
    default_input_slot,
...
from tools.function_tools.pipeline_runner.engine.outputs import default_pipeline_output_dir
from tools.function_tools.pipeline_runner.engine.paths import (
...
from tools.function_tools.pipeline_runner.engine.types import PipelineContext
...
def run_pipeline(
    ...
    if engine == "shell":
        from tools.function_tools.pipeline_runner.engine.shell import run_shell_pipeline
    ...
    if engine == "snakemake":
        from tools.function_tools.pipeline_runner.engine.snakemake import run_snakemake_pipeline

        return run_snakemake_pipeline(context, cores=cores, dry_run=dry_run)
    if engine == "nextflow":
        from tools.function_tools.pipeline_runner.engine.nextflow import run_nextflow_pipeline

        return run_nextflow_pipeline(context, cores=cores, dry_run=dry_run)
    if engine == "wdl":
        from tools.function_tools.pipeline_runner.engine.wdl import run_wdl_pipeline
