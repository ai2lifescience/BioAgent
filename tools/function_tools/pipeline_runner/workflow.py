"""Tool workflow for approved pipeline folders."""
from __future__ import annotations
from tools.function_tools.pipeline_runner.engine.runner import run_pipeline as _action_pipeline_runner
from pathlib import Path
from datetime import datetime, timezone
import json
from typing import Any
from tools.common.context import WorkflowContext, ensure_workflow_context
from tools.function_tools.pipeline_runner.engine.config import load_pipeline_config, load_runner_config
from tools.function_tools.pipeline_runner.engine.inputs import default_input_slot, pipeline_input_specs, resolve_input_value, stringify_input_value, validate_input_path, validate_input_value
from tools.function_tools.pipeline_runner.engine.paths import resolve_pipeline_dir, resolve_pipeline_name, resolve_project_path
...
def _input_line(item: dict[str, Any]) -> str:
    ...
    return f"- {label} (`{item['slot']}`, key `{config_key}`): {_accepts_text(spec)}"

def _input_example(pipeline_name: str, input_specs: dict[str, dict[str, Any]]) -> str:
    if not input_specs:
...
def _requested_input_record(item: dict[str, Any], reason: str) -> dict[str, Any]:
    ...
    record = {'slot': slot, 'label': str(spec.get('label') or slot), 'config_key': config_key, 'accepts': list(spec.get('accepts') or []), 'reason': reason}
    if item.get('path'):
...
def _requested_inputs(missing: list[dict[str, Any]], invalid: list[dict[str, Any]], input_specs: dict[str, dict[str, Any]] | None=None) -> list[dict[str, Any]]:
    ...
    for slot, spec in (input_specs or {}).items():
        if spec.get('required') or slot in seen_slots:
            continue
        requested.append(_requested_input_record({'slot': slot, 'spec': spec}, 'optional'))
    return requested

def _canonical_input_overrides(input_overrides: dict[str, Any] | None, input_specs: dict[str, dict[str, Any]]) -> tuple[dict[str, str], list[str]]:
...
def _resolve_supplied_inputs(input_path: str | None, input_overrides: dict[str, str], input_specs: dict[str, dict[str, Any]]) -> tuple[dict[str, Any], list[dict[str, Any]]]:
    ...
    for slot, path_text in input_overrides.items():
        ...
        try:
            ...
            invalid.append({'slot': slot, 'path': path_text, 'reason': str(exc), 'spec': spec})
    if input_path:
        ...
        try:
            ...
            invalid.append({'slot': slot, 'path': input_path, 'reason': str(exc), 'spec': spec})
    return (resolved, invalid)

def _required_input_check(pipeline_name: str, input_path: str | None, input_overrides: dict[str, Any] | None, config_overrides: dict[str, Any] | None=None) -> tuple[dict[str, Any] | None, dict[str, str] | None]:
    ...
    invalid_config_keys = {str((item.get('spec') or {}).get('config_key') or item.get('slot') or '') for item in invalid if isinstance(item, dict)}
    ...
    for slot, spec in input_specs.items():
        ...
        if config_key in supplied or config_key in invalid_config_keys:
            continue
        missing.append({'slot': slot, 'spec': spec, 'reason': 'No explicit path was provided. Pipeline defaults are not used by the BioAgent skill.'})
    ...
    lines: list[str] = [f'Pipeline `{resolved_name}` needs input file information before it can run.', 'BioAgent requires explicit runtime input paths and does not use pipeline default inputs.', '']
    if missing:
    ...
    if override_errors:
        ...
        lines.append('')
    lines.extend(['Upload the needed file or provide a path in the request.', '', 'Example:', _input_example(resolved_name, input_specs)])
    return ({'skill': 'pipeline_runner', 'status': 'needs_input', 'needs_input': True, 'pipeline_name': resolved_name, 'base_config_path': str(raw_config_path), 'missing_inputs': missing, 'invalid_inputs': invalid, 'input_errors': override_errors, 'requested_inputs': _requested_inputs(missing, invalid, input_specs), 'config_overrides': dict(config_overrides or {}), 'answer': '\n'.join(lines)}, canonical_overrides)

def _required_parameter_check(pipeline_name: str, config_overrides: dict[str, Any] | None) -> dict[str, Any] | None:
    ...
    lines.extend(['', f'Example: Run pipeline with pipeline_name: {resolved_name} pathogen: H1N1'])
    return {'skill': 'pipeline_runner', 'status': 'needs_parameters', 'needs_parameters': True, 'pipeline_name': resolved_name, 'required_parameters': missing, 'parameter_errors': invalid, 'answer': '\n'.join(lines)}

def _normalized_choice(value: Any) -> str:
    return ''.join((character for character in str(value or '').lower() if character.isalnum()))

def pipeline_runner(pipeline_name: str=DEFAULT_PIPELINE_NAME, input_path: str | None=None, output_dir: str | None=None, label: str | None=None, cores: int | None=None, dry_run: bool=False, timeout: int | None=None, input_overrides: dict[str, Any] | None=None, config_overrides: dict[str, Any] | None=None, context: WorkflowContext | None=None) -> dict[str, Any]:
    ...
    input_request, canonical_input_overrides = _required_input_check(pipeline_name=pipeline_name, input_path=input_path, input_overrides=input_overrides, config_overrides=config_overrides)
    if input_request:
        return input_request
    result = context.call('pipeline_runner', _action_pipeline_runner, _drop_none({'pipeline_name': pipeline_name, 'input_path': input_path, 'output_dir': output_dir, 'label': label, 'cores': cores, 'dry_run': dry_run, 'timeout': timeout, 'input_overrides': canonical_input_overrides, 'config_overrides': config_overrides, 'artifact_dir': context.workspace_dir, 'run_id': context.run_id}))['result']
    engine = str(result.get('engine', 'pipeline'))
    if result.get('status') == 'ok' and not result.get('dry_run') and result.get('run_dir'):
        completion = {
            'pipeline_name': result.get('pipeline_name', pipeline_name),
            'run_id': context.run_id,
            'completed_at': datetime.now(timezone.utc).isoformat(),
            'files': result.get('files', []),
            'output_records': result.get('output_records', []),
        }
        Path(result['run_dir'], 'pipeline-result.json').write_text(json.dumps(completion), encoding='utf-8')
    answer = f"{engine.title()} pipeline completed.\nStatus: {result.get('status', 'unknown')}\nPipeline: {result.get('pipeline_name', pipeline_name)}\nEngine: {engine}\nDry run: {result.get('dry_run', False)}\nResults are ready in the web UI as download links; previews are hidden by default.\nIf you want an interpreted view, ask: Collect and show all results from this pipeline run."
    return {'skill': 'pipeline_runner', 'tool': 'pipeline_runner', 'answer': answer, **result, 'presentation': 'downloads_only'}
