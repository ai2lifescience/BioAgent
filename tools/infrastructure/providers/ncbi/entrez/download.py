...
from tools.infrastructure.providers.ncbi.entrez.http import _base_params, _request_get, _search_ncbi
from tools.infrastructure.providers.ncbi.entrez.metadata import _download_metadata_from_history
from tools.infrastructure.providers.ncbi.entrez.naming import _query_output_stem, _term_output_prefix
from tools.infrastructure.providers.ncbi.entrez.spec import DEFAULT_BATCH_SIZE
from tools.infrastructure.workspace.paths import output_file_path
...
def _download_one_query(
    ...
    output_path = output_file_path(
        query_output_root,
        filename if filename is not None else f"{output_stem}.fasta",
    )
    metadata_path = output_file_path(
        query_output_root,
        metadata_filename if metadata_filename is not None else f"{output_stem}.metadata.csv",
    )
