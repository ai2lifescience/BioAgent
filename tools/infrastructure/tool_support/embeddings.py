"""Bounded embedding transport used by the evidence index and retriever."""
...
def embed_texts(texts: Iterable[str], model: str = DEFAULT_EMBEDDING_MODEL) -> list[list[float]]:
    ...
    with create_client() as client:
        response = client.embeddings.create(model=model, input=payload, encoding_format="float")
...
def embed_records(records: list[dict[str, Any]], model: str = DEFAULT_EMBEDDING_MODEL) -> list[dict[str, Any]]:
...
__all__ = ["embed_records", "embed_texts"]
