...
from tools.infrastructure.pipeline_engine.engine.outputs import rewrite_output_config_fields
from tools.infrastructure.pipeline_engine.engine.paths import resolve_pipeline_file
...
def load_pipeline_config(
    ...
    """Build engine defaults from the manifest and an optional native config.

    Precedence is native file < manifest defaults < manifest params. Explicit
    runtime inputs and parameter overrides are applied when the job is prepared.
    Without a separate file, runner.yaml is the configuration provenance path.
    """
    if runner_config is None:
        runner_config, _ = load_runner_config(pipeline_dir)
    config_file = (
    ...
    # Existing external pipelines may still use the conventional base config.
    # An explicitly declared file must exist; never silently ignore a typo.
    if config_file or (pipeline_dir / "config.yaml").is_file():
    ...
    if not isinstance(defaults, dict):
        raise ValueError("runner.yaml defaults must be a mapping.")
    config = _merge_defaults(config, defaults)
    if "params" in runner_config:
        params = runner_config["params"]
        if not isinstance(params, dict):
            raise ValueError("runner.yaml params must be a mapping.")
        existing = config.get("params", {})
        if not isinstance(existing, dict):
            raise ValueError("Pipeline config params must be a mapping.")
        config["params"] = _merge_defaults(existing, params)
    return config, config_path


def _merge_defaults(base: dict[str, Any], overrides: dict[str, Any]) -> dict[str, Any]:
    """Merge mappings without modifying the manifest or native file values."""
