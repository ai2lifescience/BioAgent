...
def session_root(context: WorkflowContext) -> Path:
    ...
    root = Path(context.session_dir).resolve()
...
def workspace_output_path(context: WorkflowContext, *parts: str) -> Path:
    ...
    root = Path(context.workspace_dir or context.runtime_path("outputs")).resolve()
    candidate = (root.joinpath(*parts)).resolve()
    if not candidate.is_relative_to(root):
        raise ValueError("Output paths must stay inside the session workspace.")
