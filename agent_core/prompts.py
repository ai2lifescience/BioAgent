...
def build_skill_loop_prompt(model_label: str, skill_hints: Iterable[str] = ()) -> str:
    ...
    return (
        ...
        "schema provides one. When a required detail or material preference is "
        "missing, call ask_user with a concise question and two to six concrete "
        "options. The ask_user call pauses work, so do not combine it with other "
        "skill calls. Registered skills:\n"
        f"{skill_section}\n"
...
def build_direct_response_prompt(model_label: str) -> str:
    ...
    return (
        ...
        "ask BioAgent to use the appropriate skill. If a required detail or "
        "material preference is genuinely missing, call ask_user with two to "
        "six concrete choices instead of guessing."
    )
