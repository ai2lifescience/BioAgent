"""No-network smoke check for the template bioinformatics demo pipeline."""
...
def main() -> int:
    pipeline_dir = PROJECT_ROOT / "tools" / "runtime_tools" / "pipelines" / "template_bio"
    with TemporaryDirectory(prefix="agent-template-bio-") as artifact_dir:
        result = run_pipeline(
            pipeline_name="template_bio",
        ...
        assert "Template Bioinformatics Pipeline Report" in output_paths["report.html"].read_text(
            encoding="utf-8"
        )
