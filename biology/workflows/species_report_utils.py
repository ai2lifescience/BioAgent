...
def collection_name_for(species_name: str, override: str | None=None) -> str:
    ...
    name = re.sub('[^a-zA-Z0-9._-]+', '_', raw).strip('._-')
    name = re.sub('\\.{2,}', '.', name)
    if len(name) < 3:
        name = f'kb_{name}'
    return name[:63].strip('._-') or 'species_kb'

def build_research_question(species_name: str, question: str | None, concerns: list[str] | None) -> str:
    parts = [normalize_text(question or '')]
    if concerns:
        parts.append('User concerns: ' + ', '.join((normalize_text(c) for c in concerns if c)))
    detail = ' '.join((part for part in parts if part)).strip()
    if not detail:
        detail = 'Summarize trusted knowledge, taxonomy, genome structure, host range, replication, variants or mutations, applications, and uncertainty.'
...
def source_summary(records: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for index, record in enumerate(records, start=1):
        ...
        summaries.append({'id': f'S{index}', 'source': record['source'], 'title': record['title'], 'url': record['url'], 'source_type': meta.get('source_type', ''), 'pmid': meta.get('pmid', ''), 'year': meta.get('year', '')})
    return summaries
