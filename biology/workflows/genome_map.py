...
from biology.genome_map.core import create_genome_map as _action_genome_map
...
def genome_map(fasta_path: str | None=None, genbank_path: str | None=None, gff_path: str | None=None, artifact_ref: str | None=None, layout: str='circular', label: str | None=None, min_orf_length: int=90, context: WorkflowContext | None=None) -> dict[str, Any]:
    ...
    if _should_use_latest_fasta(fasta_path=fasta_path, genbank_path=genbank_path, artifact_ref=artifact_ref):
        source_artifact = context.latest_artifact(kinds=('fasta',), suffixes=('.fasta', '.fa', '.fna', '.faa'))
    ...
    result = context.call('genome_map', _action_genome_map, {'fasta_path': fasta_path, 'genbank_path': genbank_path, 'gff_path': gff_path, 'output_dir': context.artifact_path('genome_maps'), 'label': label, 'layout': layout, 'min_orf_length': min_orf_length})['result']
