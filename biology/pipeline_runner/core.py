...
from biology.pipeline_runner.config import (
    load_pipeline_config,
    load_runner_config,
    normalize_config_overrides,
)
from biology.pipeline_runner.inputs import (
    default_input_slot,
...
from biology.pipeline_runner.outputs import default_pipeline_output_dir
from biology.pipeline_runner.paths import (
...
from biology.pipeline_runner.types import PipelineContext
...
def run_pipeline(
    ...
    if engine == "shell":
        from biology.pipeline_runner.shell import run_shell_pipeline
    ...
    if engine == "snakemake":
        from biology.pipeline_runner.snakemake import run_snakemake_pipeline

        return run_snakemake_pipeline(context, cores=cores, dry_run=dry_run)
    if engine == "nextflow":
        from biology.pipeline_runner.nextflow import run_nextflow_pipeline

        return run_nextflow_pipeline(context, cores=cores, dry_run=dry_run)
    if engine == "wdl":
        from biology.pipeline_runner.wdl import run_wdl_pipeline
