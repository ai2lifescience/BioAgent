...
from bacvf.assembly import assembler_command, trimmomatic_command
from bacvf.batch import run_samples
from bacvf.command import CommandRunner
from bacvf.config import ConfigurationError, load_config
from bacvf.input import (
...
from bacvf.prokka import prokka_command
from bacvf.qc import assembly_qc
...
def test_environment_expansion_and_relative_paths(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    data = yaml.safe_load((ROOT / "config.yaml").read_text(encoding="utf-8"))
    data["databases"]["abricate_datadir"] = "${VF_DB_ROOT}"
    data["databases"]["metadata"] = "relative/metadata.tsv"
    path = tmp_path / "deployment.yaml"
    path.write_text(yaml.safe_dump(data), encoding="utf-8")
    monkeypatch.setenv("VF_DB_ROOT", str(tmp_path / "external db"))
...
def test_dry_run_writes_contract_and_command_plan(tmp_path: Path) -> None:
    ...
    assert (output / "vf_hits.tsv").read_text(encoding="utf-8").startswith("sample_id\t")
    assert (output / "vf_hits.jsonl").read_text(encoding="utf-8") == ""
