"""End-to-end BacVF orchestration."""
...
from .abricate import VF_FIELDS, enrich_hits, parse_abricate, run_abricate
...
def run_pipeline(
    ...
    *,
    ...
    """Run BacVF and return the success status payload."""
    ...
    pipeline_log.write_text(f"START bacvf {started}\n", encoding="utf-8")
    ...
    try:
        ...
        write_tsv(output / "vf_hits.tsv", rows, VF_FIELDS)
        write_jsonl(output / "vf_hits.jsonl", rows)
        ...
        finished = utc_now()
        output_files = [
            "vf_hits.tsv", "vf_hits.jsonl", "status.json", "manifest.json", "qc.json"
        ]
        if validated.analysis_mode == "reads":
        ...
        with pipeline_log.open("a", encoding="utf-8", newline="\n") as handle:
            handle.write(f"END bacvf success {finished}\n")
        ...
        with pipeline_log.open("a", encoding="utf-8", newline="\n") as handle:
            handle.write(f"END bacvf failed {utc_now()} {type(exc).__name__}: {exc}\n")
        raise
