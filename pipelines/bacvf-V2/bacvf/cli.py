...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(
        prog="bacvf",
        description="VFDB core sequence-homology virulence-factor homologs from metagenomes.",
    )
...
def _execute_run(namespace: argparse.Namespace) -> int:
    try:
        ...
        print(f"bacvf: {type(exc).__name__}: {exc}", file=sys.stderr)
        return 1
...
def fasta_main(argv: Sequence[str] | None = None) -> int:
    ...
    parser = argparse.ArgumentParser(prog="bacvf-fasta")
...
def fastq_main(argv: Sequence[str] | None = None) -> int:
    ...
    parser = argparse.ArgumentParser(prog="bacvf-fastq")
