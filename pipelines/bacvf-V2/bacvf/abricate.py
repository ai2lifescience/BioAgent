"""ABRicate command construction and name-based VF result parsing."""
...
VF_FIELDS = [
    ...
    "supporting_reads", "depth",
    "database_name", "database_version", "locus_tag", "prokka_gene",
    "prokka_product",
]


def abricate_command(assembly: Path, config: PipelineConfig) -> list[str]:
    """Build the configured custom-VFDB-core ABRicate command."""
    tool = config.value("tools", "abricate")
    return [
        str(tool["executable"]),
        *[str(item) for item in tool.get("prefix_options", [])],
        "--datadir", str(config.database_path("abricate_datadir")),
...
def enrich_hits(
    ...
    """Join VFDB hierarchy without inferring pathogenicity."""
    enriched: list[dict[str, Any]] = []
    for hit in hits:
        raw_accession = str(hit.get("accession", ""))
        meta = metadata.get(normalize_accession(raw_accession))
        if meta is None:
            warnings.append(
                f"ABRicate hit accession '{raw_accession or '(empty)'}' "
                "did not match VFDB metadata."
            )
            meta = {}
        row = {field: "" for field in VF_FIELDS}
        row.update(hit)
        row.update(
            {
                "sample_id": sample_id,
                "accession": raw_accession if raw_accession != "." else "",
                "vf_id": meta.get("vf_id", ""),
                "gene": hit.get("gene") or meta.get("gene", ""),
                "factor_name": meta.get("factor_name", ""),
                "category": meta.get("category", ""),
                "subcategory": meta.get("subcategory", ""),
                "associated_pathogen": meta.get("associated_pathogen", ""),
