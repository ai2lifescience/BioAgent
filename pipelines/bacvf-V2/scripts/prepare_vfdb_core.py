"""Prepare a custom ABRicate database from official VFDB files."""
...
from bacvf.command import CommandRunner
from bacvf.manifest import sha256_file, utc_now
from bacvf.metadata import OUTPUT_FIELDS, load_metadata, normalize_accession
from bacvf.outputs import write_json, write_tsv
from bacvf.qc import iter_fasta
...
def prepare_database(
    ...
    *,
    ...
    if not records:
        raise ValueError("VFDB nucleotide FASTA contains no records.")
    if target.exists():
    ...
    with sequence_path.open("w", encoding="utf-8", newline="\n") as handle:
        for identifier, sequence in records:
            ...
            if row is None:
                raise ValueError(f"FASTA accession is absent from VFDB metadata: {identifier}")
            gene = _safe_header(row.get("gene") or identifier)
            resistance = _safe_header(row.get("category") or row.get("factor_name") or "virulence_factor")
            handle.write(
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    parser.add_argument("--database-name", default="vfdb_core")
