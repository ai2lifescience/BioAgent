...
from bacfunc.assembly import assembler_command, trimmomatic_command
from bacfunc.batch import run_samples
from bacfunc.command import CommandRunner
from bacfunc.config import ConfigurationError, load_config
from bacfunc.input import (
...
from bacfunc.prokka import prokka_command
from bacfunc.qc import assembly_qc
...
def config_file(tmp_path: Path) -> Path:
    ...
    data["databases"] = {"eggnog": {"data_dir": "db", "manifest": None}}
...
def test_environment_expansion_and_relative_paths(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    data = yaml.safe_load((ROOT / "config.yaml").read_text(encoding="utf-8"))
    data["databases"]["eggnog"]["data_dir"] = "${FUNC_DB_ROOT}"
    data["databases"]["eggnog"]["manifest"] = "relative/manifest.json"
    path = tmp_path / "deployment.yaml"
    path.write_text(yaml.safe_dump(data), encoding="utf-8")
    monkeypatch.setenv("FUNC_DB_ROOT", str(tmp_path / "external db"))
    config = load_config(path)
    assert config.eggnog_path("data_dir") == (tmp_path / "external db").resolve()
    assert config.eggnog_path("manifest") == (tmp_path / "relative/manifest.json").resolve()
...
def test_unset_environment_variable_is_clear(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    data = yaml.safe_load((ROOT / "config.yaml").read_text(encoding="utf-8"))
    data["databases"]["eggnog"]["data_dir"] = "${NOT_SET_FOR_TEST}"
...
def test_dry_run_writes_contract_and_command_plan(tmp_path: Path) -> None:
    ...
    assert (output / "gene_annotations.tsv").read_text(encoding="utf-8").startswith("sample_id\t")
    assert (output / "annotation_terms.tsv").read_text(encoding="utf-8").startswith("sample_id\t")
